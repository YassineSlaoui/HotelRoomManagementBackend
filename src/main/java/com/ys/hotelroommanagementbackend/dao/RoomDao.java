...
public interface RoomDao extends JpaRepository<Room, Long> {
    ...
    @Query("SELECT r FROM Room AS r WHERE " +
            ...
            "(:availabilityStartDate IS NULL OR r.availabilityStartDate >= :availabilityStartDate) AND " +
            "(:availabilityEndDate IS NULL OR r.availabilityEndDate <= :availabilityEndDate)")
    Page<Room> filterRoomsPaged(@Param("roomNumber") String roomNumber,
                                @Param("roomType") String roomType,
                                @Param("description") String description,
                                @Param("minPrice") Double minPrice,
                                @Param("maxPrice") Double maxPrice,
    ...
    @Query("SELECT DISTINCT r FROM Room r " +
            ...
            "WHERE res.guest <> :guest OR res IS NULL")
    List<Room> findNewRoomsForGuest(@Param("guest") Guest guest);

    @Query("SELECT DISTINCT r FROM Room r " +
            "LEFT JOIN r.reservations res " +
            "WHERE res.guest <> :guest OR res IS NULL")
    Page<Room> findPagedNewRoomsForGuest(@Param("guest") Guest guest, Pageable pageable);
}
